		...
		<div className="flex flex-col gap-4 lg:h-[calc(100vh-64px)] w-full mx-auto px-3 lg:w-3/5 items-center justify-center text-lg">
			...
			<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 w-full">
				{chatrooms.map((chatroom) => {
					return (
						<Link href={`/chatrooms/${chatroom.id}`} key={chatroom.id} className="mx-auto border border-accent-foreground border-md hover:shadow-md hover:scale-105 duration-150 transition-all p-3 flex items-center justify-between gap-2 w-full">
